...
<div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900/60">
  ...
  <form action="/Admin/artikel/<?= esc($artikel['id']); ?>/update" method="post" class="space-y-6" enctype="multipart/form-data">
    ...
    <div>
      ...
      <?php else: ?>
        <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">Biarkan kosong jika tidak ingin mengganti gambar. Format yang diizinkan: JPG, PNG, atau WEBP.</p>
      <?php endif; ?>
      <?php if (! empty($artikel['gambar'])): ?>
        ...
        <div class="mt-4">
          ...
          <button type="button" data-admin-lightbox
            ...
            <span class="sr-only">Lihat versi besar gambar artikel saat ini</span>
            <img src="<?= esc($currentImage); ?>" alt="Gambar artikel saat ini" class="h-32 w-32 object-cover" />
          </button>
        </div>
      <?php endif; ?>
    </div>
