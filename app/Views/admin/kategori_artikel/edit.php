...
<div
  class="mx-auto max-w-3xl rounded-2xl border border-gray-200 bg-white p-8 shadow-sm dark:border-gray-800 dark:bg-gray-900/60">
  <div class="mb-6 flex items-center justify-between">
    ...
    <a href="/Admin/kategori-artikel" class="text-sm font-medium text-indigo-600 hover:text-indigo-500">&larr; Kembali
      ke daftar</a>
  </div>
  ...
  <?php if (! empty($errors)): ?>
  <div
    class="mb-6 rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700 dark:border-red-900/60 dark:bg-red-900/30 dark:text-red-200">
  ...
  <form action="/Admin/kategori-artikel/<?= esc($kategori['id']); ?>/update" method="post" class="space-y-6">
    ...
    <div class="flex items-center justify-end gap-3">
      ...
      <button type="submit"
        class="inline-flex items-center rounded-lg bg-indigo-600 px-5 py-2 text-sm font-semibold text-gray-700 shadow-sm transition hover:bg-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Simpan
        Perubahan</button>
    </div>
  </form>
</div>
