...
<div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900/60">
  ...
  <form method="get" action="/Admin/kategori-artikel" class="mb-6">
    <div class="relative flex items-center">
      ...
      <?php if ($search): ?>
        <a href="/Admin/kategori-artikel" class="absolute right-3 text-xs font-semibold uppercase text-indigo-600 hover:text-indigo-500">
          Reset
        </a>
      <?php endif; ?>
    </div>
  ...
  <div class="overflow-x-auto rounded-xl border border-gray-200 shadow-sm dark:border-gray-800">
    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
      <thead class="bg-gray-50 dark:bg-gray-800/70">
        <tr>
          <th scope="col" class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-300">Nama Kategori</th>
          <th scope="col" class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-300">Slug</th>
          <th scope="col" class="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-300">Dibuat</th>
          <th scope="col" class="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-300">Aksi</th>
        </tr>
      </thead>
      <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-800 dark:bg-gray-900/40">
        <?php if (! empty($categories)): ?>
          <?php foreach ($categories as $kategori): ?>
            <tr class="transition hover:bg-gray-50/70 dark:hover:bg-gray-800/60">
              ...
              <td class="px-6 py-4 text-right text-sm">
                <div class="flex justify-end gap-2">
                  ...
                  <form action="/Admin/kategori-artikel/<?= esc($kategori['id']); ?>/delete" method="post" onsubmit="return confirm('Hapus kategori ini?');">
                    <?= csrf_field(); ?>
                    <button type="submit"
