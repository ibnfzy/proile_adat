...
<div class="mx-auto max-w-3xl rounded-2xl border border-gray-200 bg-white p-8 shadow-sm dark:border-gray-800 dark:bg-gray-900/60">
  <div class="mb-6 flex items-center justify-between">
    <div>
      <h3 class="text-2xl font-semibold text-gray-800 dark:text-white">Tambah Kategori Artikel</h3>
      <p class="mt-1 text-sm text-gray-500 dark:text-gray-300">Masukkan informasi kategori baru untuk artikel.</p>
    </div>
  ...
  <form action="/Admin/kategori-artikel/store" method="post" class="space-y-6">
    <?= csrf_field(); ?>
    <div>
      <label for="nama" class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-200">Nama Kategori</label>
      <input type="text" id="nama" name="nama" value="<?= esc(old('nama')); ?>"
    ...
    <div class="flex items-center justify-end gap-3">
      ...
      <button type="submit"
        class="inline-flex items-center rounded-lg bg-indigo-600 px-5 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Simpan</button>
    </div>
  </form>
</div>
<?= $this->endSection(); ?>
