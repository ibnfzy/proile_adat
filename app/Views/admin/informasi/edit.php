...
<div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900/60">
  ...
  <form action="/Admin/informasi/<?= esc($informasi['id']); ?>/update" method="post" class="space-y-6" enctype="multipart/form-data">
    <?= csrf_field(); ?>
    ...
    <div>
      ...
      <?php else: ?>
        <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">Kosongkan jika tidak ingin mengganti gambar. Format yang didukung: JPG, PNG, atau WEBP.</p>
      <?php endif; ?>
      <?php if (! empty($informasi['gambar'])): ?>
        <?php
        $currentImage = $informasi['gambar'];
