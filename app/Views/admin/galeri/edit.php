...
<div class="mx-auto max-w-3xl rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900/60">
  ...
  <form action="/Admin/galeri/<?= esc($photo['id']); ?>/update" method="post" enctype="multipart/form-data" class="space-y-6">
    ...
    <div>
      <label for="gambar" class="mb-2 block text-sm font-semibold text-gray-700 dark:text-gray-200">Ganti Foto (Opsional)</label>
    ...
    <div>
      <label for="video" class="mb-2 block text-sm font-semibold text-gray-700 dark:text-gray-200">Unggah Video Baru (Opsional)</label>
    ...
    <?php if (! empty($photo['video'])): ?>
      <div class="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-900/40">
        <p class="mb-3 text-sm font-semibold text-gray-700 dark:text-gray-200">Video Saat Ini</p>
        ...
        <label class="mt-3 inline-flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
          <input type="checkbox" name="remove_video" value="1" class="h-4 w-4 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500">
